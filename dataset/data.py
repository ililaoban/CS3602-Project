...
class MyDataset(Dataset):
    ...
    def __init__(self, arguments,data_path, label_converter: LabelConverter, model_name: str, cache_dir):
        os.makedirs(cache_dir, exist_ok=True)
        md5 = hashlib.md5((model_name + data_path + cache_dir).encode('utf-8')).hexdigest()
        cache_path = Path(cache_dir) / md5
        if arguments.device != -1:
            device = 'cuda'
        if cache_path.is_file():
            self._data = torch.load(cache_path, map_location=device)
            return
        ...
        print("begin iteration initilization")
        ...
        for i in tqdm(data):
            ...
            for j in i:
                ...
                vector_with_noise = output[0][0].to(device).detach()
                model_input = tokenizer(text_without_noise, return_tensors='pt')
                output = model(**model_input)
                vector_without_noise = output[0][0].to(device).detach()
                ...
                for i2, v2 in enumerate(bio_labels):
                    tensor[i2, v2] = 1
                labels[k] = tensor.to(device)
            self._data.append((features, labels))
        print("finish initilization iteration")
