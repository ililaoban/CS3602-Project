...
def pinyin_denoise(predictions, select_pos_set, select_others_set, example_map_dic, example_pinyin_set):
    ...
    for i in range(len(predictions)):
        ...
        tmp_pinyin = p.get_pinyin(tmp_pred[2])
        if tmp_pred[1] != "value":
            if tmp_pred[1] in select_pos_set:
                map_dic, pinyin_set = example_map_dic, example_pinyin_set
            else:
                [map_dic, pinyin_set] = select_others_set[tmp_pred[1]]
            tmp_pinyin = tmp_pinyin.replace("-", ' ')
            standard_output = get_standard_output(map_dic, pinyin_set, tmp_pinyin)
            if standard_output == None:
                continue
            predictions[i] = [tmp_pred[0], tmp_pred[1], standard_output]
    return predictions
